...
export default {
  ...
  methods: {
    ...
    removeCategoryName() {
      ...
      // 将商品分类Id置空(设置为undefined，发请求时，不会携带，减少带宽的开销)
    ...
    removeKeyword() {
      ...
      // 通知Header组件中的输入框置空，组件间通信，全局事件总线
    ...
    removeAttrValue(index) {
      // 通过index来删除props数组中的元素
      this.searchParams.props.splice(index, 1)
      // 发送请求
      this.getData()
    },
    // 用来响应子组件的自定义事件，根据品牌名发送请求
    trademarkInfo(value) {
      // 将品牌名赋值
  ...
  // 组件挂载完毕，执行一次，只执行一次
  mounted() {
    // 发请求之前带给服务器参数【searchParams对象里的参数发生变化，有数值带给服务器】
    this.getData()
    // 把以前的分类ID置空
